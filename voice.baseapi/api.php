...
substring(replace(replace(replace(now(),'-',''),':',''),' ',''),1,14) , '$uploadfile','$roletomove','$confidence','$distance','$threshold','$userid');") or die(mysql_error()); 
							...
							$characters1 = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
								...
								$randnumber1 = $randomString1;
									...
									$query = mysql_query("INSERT INTO voice.tvsets 
													...
													VALUES
													('$inputtvid','$randnumber1',  substring(replace(replace(replace(now(),'-',''),':',''),' ',''),1,14) );") or die(mysql_error());
	...
	public function recognition_alreadyexist(&$inputtvid, &$uploadfile,&$url,&$threshold,&$distance)
    {
			...
			//$urls = 'localhost/api.voice/v0/user_recognition';
              ...
              /*   curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                ...
                $function_called = 'recognize voice';
                $user_id_log = $inputtvid;
                $message = 'Internal API call end repeated : '.$output;
                $this->log_table($function_called, $user_id_log, $message); 
                $json     = json_decode($output);
                $text     = json_encode($json, JSON_PRETTY_PRINT);
                $fff      = str_replace('[', '', $output);
   ...
   private function base_get_user_list()
    {
       ...
       if ($this->get_request_method() != "POST")
				...
				$sql = mysql_query("SELECT user_name as user_name,user_role,userid as user_id FROM voice.voice_register_tbl where tvid = '$inputtvid'  order by user_id");
				if (mysql_num_rows($sql) > 0)
				{
					 $function_called = 'get_user_details';
						$user_id_log = $inputtvid;
...
private function base_user_deletion()

    {
		...
		$user_id      = trim($_GET['user_id']);
			...
			$tvstatus     = strtolower(trim($_GET["tvstatus"]));

			
			if ((strcmp($tvstatus,'old')==0) || empty($tvstatus) )
			{
				$tvstatus = 'old';
			}
...
$query2 = mysql_query("SELECT tvid as tv_id
            ...
            $this->response($this->json($error), 485);
				...
				$function_called = 'base_user_deletion';
                $user_id_log     = $inputtvid;
                $message         = 'Internal API call tvstatus value'. $tvstatus;
                
                $this->log_table($function_called, $user_id_log, $message);
				
				
													if ((strcmp($tvstatus,'new')==0) )
			{					
                curl_setopt($ch, CURLOPT_URL, 'localhost/api.voice2t/v0/user_registration');
				
				
				$function_called = 'base_user_deletion';
                $user_id_log     = $inputtvid;
                $message         = 'Internal API call  del new';
                $this->log_table($function_called, $user_id_log, $message);
			...
			else
			{
				 ...
				 $function_called = 'base_user_deletion';
                $user_id_log     = $inputtvid;
                $message         = 'Internal API call  del old';
                $this->log_table($function_called, $user_id_log, $message);
	 ...
	 private function base_max_user_count()
    {
 ...
 private function base_user_registration()
    {
		...
		$tvstatus     = strtolower(trim($_GET["tvstatus"]));
			...
			$rereg     = strtolower(trim($_GET["rereg"]));

			
			if ((strcmp($rereg,'false')==0) || empty($rereg) )
			{
				$rereg = 'false';
			}
      ...
      //  $reg_name      = 'zhh4563';
        ...
        if (isset($_FILES["audiofile"]))
        {
            ...
            if (move_uploaded_file($_FILES['audiofile']['tmp_name'], $uploadfile))
            {
				...
				// check if it already registred
				
					if ((strcmp($rereg,'true')==0) )
			{
			...
			$function_called = 'user registration';
				...
				$querynew = mysql_query("SELECT count(userid) as userid
										   FROM voice.voice_register_tbl 
										   WHERE tvid = '$inputtvid'
											") or die(mysql_error());
											$row2 = mysql_fetch_array($querynew) or die(mysql_error());
											$countregeredperson = $row2['userid']; 
				
				
				
				
				if ($countregeredperson >0)
				
				{		
			$serverIP3 = "anyone/$userIdforfile/recognize/temp/";
            $old_umask = umask(0);
           ...
           // mkdir($serverIP2, 0777, true);
            $uploaddirdest      = "anyone/$userIdforfile/recognize/temp/";
            ...
            $d              = new DateTime(date('Y-m-d H:i:s.' . $micro, $t));
            $dttm1          = $d->format("YmdHisu");
            $uploadfiledest     = $uploaddirdest . $dttm1 . "." . $extension_test;
            $file_name      = basename($_FILES['audiofile']['name']);
            $dttm_now       = date("YmdHis");
            if (copy($uploadfile, $uploadfiledest))
            {
						$randomnumber = rand(0, 5);
 
						$servers= array('localhost/api.voice1/v0/user_recognition');
 						 array_push($servers,'localhost/api.voice2/v0/user_recognition');
						 array_push($servers,'localhost/api.voice3/v0/user_recognition');
						 array_push($servers,'localhost/api.voice4/v0/user_recognition');
						 array_push($servers,'localhost/api.voice5/v0/user_recognition');
						 array_push($servers,'localhost/api.voice6/v0/user_recognition');
					 
 
 					$urlrereg =$servers[$randomnumber];
					
					$function_called = 'recognize voice server during registration';
                    $user_id_log     = $userIdforfile;
					$randlog =  $randomnumber +1;
                    $message         = 'recognition at server '. $randlog ;
                    $this->log_table($function_called, $user_id_log, $message);
				
				$inputtvidrereg= $inputtvid;
				$uploadfilerereg= $uploadfiledest;
				$thresholdforapirereg = '10';
				$distancerereg = $distance;
						
					$arrayoutput             = $this->recognition_alreadyexist($inputtvidrereg, $uploadfilerereg, $urlrereg,$thresholdforapirereg,$distancerereg);
					
					$status = $arrayoutput['status'];
					$output = $arrayoutput['output'];
					$httpcode = $arrayoutput['httpcode'];
					$name = $arrayoutput['name'];
					//$role11 = $arrayoutput['role'];
...
//new code for maximum 3 users
				$maxuser = 3;
				$querynew = mysql_query("SELECT count(userid) as countuserid
										   FROM voice.voice_register_tbl 
										   WHERE tvid = '$inputtvid'
											") or die(mysql_error());
											$row2 = mysql_fetch_array($querynew) or die(mysql_error());
											$count134 = $row2['countuserid']; 
				...
				if($count134> $maxuser)
				{
								...
								$querryoldestuser = mysql_query("SELECT userid,user_name,user_role  FROM voice.voice_register_tbl 
										   WHERE tvid = '$inputtvid'
											order by userid limit 1") or die(mysql_error());
											$row2 = mysql_fetch_array($querryoldestuser) or die(mysql_error());
											$oldestuserid = $row2['userid']; 
											$oldestuserregname = $row2['user_name']; 
											$oldestuserrole = $row2['user_role']; 											
								
//code to delete 


								$function_called = 'delete max user';
								$user_id_log     = $oldestuserid;
								$message         = 'starts';
								$this->log_table($function_called, $user_id_log, $message);
								$ch   = curl_init();
								$data = array(
									'username' => $oldestuserregname,
									'tv_id' => $inputtvid,
									'job' => 'del',
									/* 'pictureuid' => $wav_uid, */
									'audiofile' => '@' . 'test.wav',
									'user_role' => $oldestuserrole
								);
								curl_setopt($ch, CURLOPT_URL, 'localhost/api.voice2t/v0/user_registration');
								curl_setopt($ch, CURLOPT_POST, 1);
								$httpcode        = curl_getinfo($ch, CURLINFO_HTTP_CODE);
								$function_called = 'delete max user';
								$user_id_log     = $userIdforfile;
								$message         = 'Internal API call middle del: ' . $oldestuserregname . "$uploadfile : " ;
								$this->log_table($function_called, $user_id_log, $message);
								// CURLOPT_SAFE_UPLOAD defaulted to true in 5.6.0
								// So next line is required as of php >= 5.6.0
								curl_setopt($ch, CURLOPT_SAFE_UPLOAD, false);
								...
								$function_called = 'delete max user';
								$user_id_log     = $userIdforfile;
								$message         = 'Internal API call end with 3 max: ' . $fff;
								$this->log_table($function_called, $user_id_log, $message);
								
								
								$function_called = 'delete max user';
								$user_id_log     = $userIdforfile;
								$message         = 'Internal API call del end with 3 max: ' . $this->json($output);
								$this->log_table($function_called, $user_id_log, $message);
...
// total end with new tv	
	...
	if(($count3 == 0)&& !(strcmp($dataflag,'true')==0)) 
	{
		...
		$function_called = 'base_user_recognition';
                $user_id_log     = $inputtvid;
                   $message         = 'data flag : false , distance: ' . $distance;
              //  $this->log_table($function_called, $user_id_log, $message);
          ...
          //  mkdir($serverIP2, 0777, true);
			...
			else
			{
				$error = array(
											'status' => "Failed",
											"msg" => "No user exist and upload failed"
										);
										$this->response($this->json($error), 461);
		  ...
		  if (isset($_FILES["audiofile"]))
        {
			 ...
			 $function_called = 'base_user_recognition';
                $user_id_log     = $inputtvid;
                $message         = 'Data received with flag true'. 'distance: ' .$distance ;
              //  $this->log_table($function_called, $user_id_log, $message);
           ...
           // mkdir($serverIP2, 0777, true);
			...
			{					
               $randomnumber = rand(0, 5);
					...
					$servers= array('localhost/api.voice1t/v0/user_recognition');
 
					
						 
						 array_push($servers,'localhost/api.voice2t/v0/user_recognition');
						 array_push($servers,'localhost/api.voice3t/v0/user_recognition');
						 array_push($servers,'localhost/api.voice4t/v0/user_recognition');
						 array_push($servers,'localhost/api.voice5t/v0/user_recognition');
						 array_push($servers,'localhost/api.voice6t/v0/user_recognition');
                    ...
                    $user_id_log     = $userIdforfile;
					$randlog =  $randomnumber +1;
                    $message         = 'recognition at new server '. $randlog ;
                    $this->log_table($function_called, $user_id_log, $message);
			...
			else
			{
                    ...
                    $user_id_log     = $userIdforfile;
					$randlog =  $randomnumber +1;
                    $message         = 'recognition at old server '. $randlog ;
                    $this->log_table($function_called, $user_id_log, $message);
			...
			//	 $urls = 'localhost/api.voice2/v0/user_recognition';
				...
				/* 	//test
					...
					if($randomnumber==5)
					{
						...
						$function_called = 'recognize voice server';
                    $user_id_log     = $userIdforfile;
                    $message         = 'recognition at server 1 ' .$newurl ;
                    $this->log_table($function_called, $user_id_log, $message); 
				...
				$arrayoutput             = $this->recognition($inputtvid, $uploadfile, $url,$thresholdforapi,$distance);
					...
					else
					{
						...
						$randomnumbernewserver = $randomnumber +1;
						$newurl =$servers[$randomnumbernewserver]; 
						$function_called = 'recognize voice server';
                    $user_id_log     = $userIdforfile;
                    $message         = 'recognition at server  . '. $randomnumber +1 . $servers[$randomnumbernewserver];
                    $this->log_table($function_called, $user_id_log, $message); 
			...
			else
			{
               ...
               // $message         = 'Internal API call middle del: ' . $reg_name . "uploadfile : " . $uploadfile . "user id : " . "$userIdforfile";
				...
				$function_called = 'base_clear_users';
														...
														$function_called = 'base_clear_users';
														$user_id_log = $inputtvid;
														$message = 'status (1 is ok, 0 not) : '.$status;
														//$this->log_table($function_called, $user_id_log, $message);
				...
				if($status == $i)
				{
					$function_called = 'base_clear_users';
														$user_id_log = $inputtvid;
														$message = 'deletion success , total: '.$i. 'delteted' .$status;
														$this->log_table($function_called, $user_id_log, $message);
														//$this->log_table($function_called, $user_id_log, $message); 
					$success = array(
											'status' => "success",
				...
				else
				{
					$function_called = 'base_clear_users';
														$user_id_log = $inputtvid;
														$message = 'deletion unsuccess , total: '.$i. 'delteted' .$status;
														$this->log_table($function_called, $user_id_log, $message);
